    ...
    override fun main() {
        ...
        patientsByMrn.forEach { entry ->
            ...
            val executionTime = measureTimeMillis {
                val run = runCatching {
                    val bundle = conditionService.findConditionsWithStaging(
                        tenant,
                        fhirId
                    )
                    val conditionCount = bundle.entry
        ...
        logger.info { "Execution time average per patient: ${ if (patientCount > 0) { totalTime / patientCount } else "N/A" } ms" }
        logger.info { "Execution time average per patient with staging Observations: ${ if (totalPatientHadStaging > 0) { totalTimeStaging / totalPatientHadStaging } else "N/A" } ms" }
        logger.info { "Execution time average per patient without staging Observations: ${ if (totalPatientNoStaging > 0) { totalTimeNoStaging / totalPatientNoStaging } else "N/A" } ms" }
        logger.info { "$totalPatientHadStaging of $patientCount patients had staging Observations (${ if (patientCount > 0) { ((totalPatientHadStaging * 100) / patientCount) } else "N/A" }%)." }
